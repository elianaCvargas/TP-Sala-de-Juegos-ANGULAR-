...
export class PiedraPapelTijeraComponent  implements OnInit {
  @Output() enviarJuego: EventEmitter<JuegoPiedraPapelTijera> = new EventEmitter();
  ...
  constructor() {
    this.nuevoJuego = new JuegoPiedraPapelTijera('Nombre', false, 'username');
  ...
  seleccionarOpcion(seleccion: number){
    ...
    this.nuevoJuego.seleccionNombre = seleccion === 1 ? 'Piedra' : seleccion === 2 ? 'Papel' : 'Tijera';
  }
  ...
  verificar() {
    ...
    if (this.nuevoJuego.verificar()) {
      ...
      this.enviarJuego.emit(this.nuevoJuego);
      this.esGanador = false;
      this.MostarMensaje('Lastima, te falto aceitar esa suerte', this.esGanador);
    }
